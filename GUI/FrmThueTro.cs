...
namespace GUI
{
    public partial class FrmThueTro : Form
    {
        ...
        private void simpleButton7_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    if(item.GetType()==typeof(CheckBox)&&item.Checked==true)
            //    {
        ...
        public void ThemCTPHIEUTHU()
        {
            ...
            //// MessageBox.Show("Mã thu của phiếu thu vừa tạo "+qlnt.PHIEUTHUTIENs.Select(t => t).OrderByDescending(e => e.MAPHIEUTHUE).FirstOrDefault().MATHU.ToString());
            //xl.TaoCTTPTHU(1);
            //xl.TaoCTTPTHU(2);
            //foreach (CheckBox item in flowLayoutPanel1.Controls)
            //{
            //    if (item.Checked)
            //    {
            //        xl.TaoCTTPTHU(xl.getMadv(item.Text));
        ...
        {
            if (textEdit1.Text == "" || textEdit3.Text == "" || textEdit4.Text == "" || textEdit5.Text == "" || textEdit6.Text == "")
            {
           ...
           if(!KiemTraDangKyItNhat1Dv())
           {
            ...
            if(checkEdit1.Checked==false)
            {
               ...
               if (radioButton1.Checked == true)
               {
                    try
                    {
                        //DialogResult dialogResult = MessageBox.Show("Bạn có in hợp đồng thuê trọ", "In hợp đồng", MessageBoxButtons.YesNo);
                        ...
                        //{
                        //}
                        xl.LapPhieuThue(int.Parse(comboBox6.SelectedValue.ToString()), int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()), DateTime.Now, true);
                ...
                else if (radioButton2.Checked == true)
                {
                    try
                    {
                        xl.LapPhieuThue(int.Parse(comboBox6.SelectedValue.ToString()), int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()), DateTime.Now, false);
                        xl.CapNhatSoLuongnguoi(int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()));
            ...
            if(checkEdit1.Checked==true)
            {
                ...
                try
                {
                    xl.ThemKTV2(dateTimePicker1.Value, textEdit1.Text, textEdit3.Text, comboBox1.Text, textEdit4.Text, textEdit5.Text, textEdit6.Text, ktch(), buttonEdit1.Text);
                }
                catch (Exception)
                {
                ...
                if (radioButton1.Checked == true)
                {
                    try
                    {
                        ...
                        //{
                        //}
                        xl.LapPhieuThue(xl.GetLastRecordKhachTro(), int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()), DateTime.Now, true);
                        xl.LapPhieuThu(DateTime.Now);
                        comboBox6.DataSource = xl.loadkt();
                        comboBox6.DisplayMember = "HOTEN";
                        comboBox6.ValueMember = "MAKT";
                ...
                else if (radioButton2.Checked == true)
                {
                    try
                    {
                        xl.LapPhieuThue(xl.GetLastRecordKhachTro(), int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()), DateTime.Now, false);
                        xl.CapNhatSoLuongnguoi(int.Parse(dataGridView1.CurrentRow.Cells[0].Value.ToString()));                     
                        MessageBox.Show("Thuê trọ thành công");
                        this.Close();
